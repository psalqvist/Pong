...
#define DRAW_POINT(x,y) game[(y)/8*COLUMNS + x] |= 1 << (y - (y/8)*8)
...
// 		game[(b.y)/8 * COLUMNS + b.x] |= 1 << b.y - b.y/8*8;
// }
//
// void draw_player(Player p) {
// 	int i;
// 	for(i = 0; i < p.height; i++) {
// 			game[(p.y+i)/8 * COLUMNS + p.x] |= 1 << ((p.y + i) - ((p.y+i)/8)*8);
// 	}
// }
...
Render the game
*/
void draw_game(Player *player1, Player *player2, Ball *ball) {
	reset_display();
	draw_player(*player1);
	draw_player(*player2);
	draw_middle();
	draw_ball(*ball);
	draw_display(game);
	//display_debug(&((*p).height));
}

/*
Helper function to get button values
*/
int btns;
int getbtns() {
  int btns_d;
...
{
  /*
	Initialize buttons 1-4 as input
	*/
	TRISF |= 0x0002;		// BTN1, bit 1 on TRISF as input
  TRISD |= 0x00e0;		// BTN2-4, bit 5 through 7 as input
